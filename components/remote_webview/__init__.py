...
RemoteWebViewPauseAction  = ns.class_("RemoteWebViewPauseAction",  automation.Action)  # ← ns thay vì remote_webview_ns
RemoteWebViewResumeAction = ns.class_("RemoteWebViewResumeAction", automation.Action)  # ← ns thay vì remote_webview_ns
...
@automation.register_action(
    "remote_webview.pause",
    RemoteWebViewPauseAction,
    cv.Schema({cv.GenerateID(): cv.use_id(RemoteWebView)}),
    synchronous=True,   # ← thêm dòng này
)
async def remote_webview_pause_action(config, action_id, template_arg, args):
    var = await cg.get_variable(config[CONF_ID])
    action = cg.new_Pvariable(action_id, template_arg)
    cg.add(action.set_parent(var))
    return action


@automation.register_action(
    "remote_webview.resume",
    RemoteWebViewResumeAction,
    cv.Schema({cv.GenerateID(): cv.use_id(RemoteWebView)}),
    synchronous=True,   # ← thêm dòng này
)
async def remote_webview_resume_action(config, action_id, template_arg, args):
    var = await cg.get_variable(config[CONF_ID])
    action = cg.new_Pvariable(action_id, template_arg)
    cg.add(action.set_parent(var))
    return action